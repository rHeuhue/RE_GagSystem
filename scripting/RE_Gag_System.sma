...
new Array:g_aList[eLists], g_iTotalPhrases[eLists], g_szName[MAX_PLAYERS + 1][MAX_NAME_LENGTH], g_szIP[MAX_PLAYERS + 1][MAX_IP_LENGTH]

// New cvar settings
new gp_blHudEnabled, gp_blEnableGagExpireMsg,
	gp_iAutoGagTime_BadWords, gp_iAutoGagTime_Advertise, 
...
public CommandSayExecuted(id)
{
	...
	{
		GagUser(g_szName[id], g_szIP[id], gp_iAutoGagTime_Advertise, fmt("%s", gp_szReasonAdvertise), gp_szAdminAdvertise);
	...
	if (g_iTotalPhrases[BLACK] > 0 && IsUserGagged(id, false) == GAG_NOT)
	{
		for (new i = 0; i < g_iTotalPhrases[BLACK]; i++)
		{
			...
			{
				GagUser(g_szName[id], g_szIP[id], gp_iAutoGagTime_BadWords, fmt("%s [%s]", gp_szReasonBadWords, szMessage), gp_szAdminBadWords)
				return PLUGIN_HANDLED;
			}
		}
	}
